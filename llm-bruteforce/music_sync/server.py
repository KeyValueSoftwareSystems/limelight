    ...
    def load(self, name, force=False):
        ...
        with self.lock:
            ...
            self.meta = {
                ...
                "plan": doc.get("plan"),                 # score-rendered shows: cues with their effects + event activity
                "score_bar_shift": doc.get("score_bar_shift"),
...
def effects_catalogue():
    """The whole effect library, one row per effect, for the page's library card."""
    return [{"name": e.name, "layer": e.layer, "suits": e.suits, "doc": e.doc}
            for layer in effects.LAYERS for e in effects.by_layer(layer)]
...
def make_handler(state: State):
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            ...
            if path == "/api/effects":
                return self._json({"effects": effects_catalogue()})
