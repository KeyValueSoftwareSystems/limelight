...
module.exports = function bounce(params, ctx) {
  ...
  for (let i = 0; i < N; i++) {
    ...
    H.setHead(f, H.HEADS[0], { level: k * 0.8, colour: c, pan: 0.55, tilt: 0.45 });
    H.setHead(f, H.HEADS[1], { level: k * 0.8, colour: c, pan: 0.77, tilt: 0.45 });
    H.setHead(f, H.HEADS[2], { level: k * 0.6, colour: c, pan: 0.62, tilt: 0.48 });
    H.setHead(f, H.HEADS[3], { level: k * 0.6, colour: c, pan: 0.70, tilt: 0.48 });
    frames.push(f);
  }
