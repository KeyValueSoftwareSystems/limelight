...
module.exports = function trade(params, ctx) {
  ...
  if (params.travel) {
    ...
    for (let i = 0; i < total; i++) {
      ...
      H.setHead(f, H.HEADS[0], { level: 0.55, colour: c0, pan: 0.20 + 0.5 * pos, tilt: 0.4 });
      if (params.head !== false) 
      H.setHead(f, H.HEADS[1], { level: 0.55, colour: c0, pan: 0.40 + 0.5 * pos, tilt: 0.4 });
      if (params.head !== false) 
      H.setHead(f, H.HEADS[2], { level: 0.40, colour: c0, pan: 0.30 + 0.4 * pos, tilt: 0.45 });
      if (params.head !== false) 
      H.setHead(f, H.HEADS[3], { level: 0.40, colour: c0, pan: 0.35 + 0.4 * pos, tilt: 0.45 });
      frames.push(f);
    }
  ...
  for (let i = 0; i < total; i++) {
    ...
    if (params.head !== false) 
    H.setHead(frame, H.HEADS[2], { level: aLv * 0.7, colour: aCol, pan: rl ? 0.62 : 0.38 });
    if (params.head !== false) 
    H.setHead(frame, H.HEADS[3], { level: bLv * 0.7, colour: bCol, pan: rl ? 0.38 : 0.62 });
    frames.push(frame);
  }
