...
module.exports = function breathe(params, ctx) {
  ...
  for (let i = 0; i < N; i++) {
    ...
    H.setHead(f, H.HEADS[0], {
      level: k * 0.8, colour,
      pan: 0.55 + 0.05 * Math.sin(2 * Math.PI * (i / N)),
      tilt: 0.46 + 0.03 * Math.cos(2 * Math.PI * (i / N)),
    });
    H.setHead(f, H.HEADS[1], {
      level: k * 0.8, colour,
      pan: 0.77 - 0.05 * Math.sin(2 * Math.PI * (i / N)),
      tilt: 0.46 + 0.03 * Math.cos(2 * Math.PI * (i / N)),
    });
    H.setHead(f, H.HEADS[2], {
      level: k * 0.6, colour,
      pan: 0.62 + 0.04 * Math.cos(2 * Math.PI * (i / N)),
      tilt: 0.50 + 0.02 * Math.sin(2 * Math.PI * (i / N)),
    });
    H.setHead(f, H.HEADS[3], {
      level: k * 0.6, colour,
      pan: 0.70 - 0.04 * Math.cos(2 * Math.PI * (i / N)),
      tilt: 0.50 + 0.02 * Math.sin(2 * Math.PI * (i / N)),
    });
    frames.push(f);
  }
