...
module.exports = function shift(params, ctx) {
  ...
  for (let i = 0; i < N; i++) {
    ...
    H.setHead(f, H.HEADS[0], { level: level * 0.85, colour: c, pan: 0.55, tilt: 0.45 });
    H.setHead(f, H.HEADS[1], { level: level * 0.85, colour: c, pan: 0.77, tilt: 0.45 });
    H.setHead(f, H.HEADS[2], { level: level * 0.65, colour: c, pan: 0.62, tilt: 0.48 });
    H.setHead(f, H.HEADS[3], { level: level * 0.65, colour: c, pan: 0.70, tilt: 0.48 });
    frames.push(f);
  }
