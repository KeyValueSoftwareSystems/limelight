...
module.exports = function bump(params, ctx) {
  ...
  for (let i = 0; i < N; i++) {
    ...
    H.setHead(f, H.HEADS[0], { level: env, colour, pan: 0.55, tilt: 0.46 });
    H.setHead(f, H.HEADS[1], { level: env, colour, pan: 0.77, tilt: 0.46 });
    H.setHead(f, H.HEADS[2], { level: env * 0.8, colour, pan: 0.62, tilt: 0.50 });
    H.setHead(f, H.HEADS[3], { level: env * 0.8, colour, pan: 0.70, tilt: 0.50 });
    frames.push(f);
  }
