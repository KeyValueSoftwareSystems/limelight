...
const BEAM_HOLD_BARS = 2;      // a beam look lasts a phrase, not a frame
...
function beamPick(ki, i){
  // deterministic, and mirrored pairs come up together so the rig stays symmetric
  const p = Math.floor(ki/2);
  let h = (Math.imul(p, 73856093) ^ Math.imul(i, 19349663)) | 0;
  h = Math.imul(h ^ (h >>> 13), 1274126177) | 0;
  h = h ^ (h >>> 16);
  return ((h >>> 0) % 1024) / 1024;
}
function beamEnv(ki, t, want){
  if(!(want > 0)) return 0;
  const m = mpos(t)/BEAM_HOLD_BARS, k = Math.floor(m), f = m - k;
  const on = i => beamPick(ki, i) < want;
  if(!on(k)) return 0;
...
;
    ...
    const lvlOut = narrow
      ? beamEnv(G.ki, t, BEAM_WANT[L] === undefined ? 0.30 : BEAM_WANT[L])
