...
module.exports = function tint(params, ctx) {
  ...
  for (let i = 0; i < N; i++) {
    ...
    H.setHead(f, H.HEADS[0], { level: level * 0.8, colour: c, pan: 0.58, tilt: 0.47 });
    H.setHead(f, H.HEADS[1], { level: level * 0.8, colour: c, pan: 0.74, tilt: 0.47 });
    H.setHead(f, H.HEADS[2], { level: level * 0.6, colour: c, pan: 0.62, tilt: 0.50 });
    H.setHead(f, H.HEADS[3], { level: level * 0.6, colour: c, pan: 0.70, tilt: 0.50 });
    frames.push(f);
  }
