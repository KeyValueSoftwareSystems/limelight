...
/* Section labels come in more than one vocabulary. A map authored in this repo
   speaks the recipe's own words (verse, drop, break); a map measured by allin1
   speaks its analyser's (chorus, inst, start), and the librosa fallback speaks a
   third (low, mid, high). The recipe used to recognise only the first, so every
   chorus a listener labelled fell through to `idle` and the room went dark on the
   biggest part of the song. Translate the FOREIGN words to the recipe's own here;
   words already in the recipe's vocabulary are left exactly as they are, so a map
   authored against this file is unchanged. Teaching the reader another dialect is
   the reader's job -- the map is allowed to speak its analyser's language. */
const CH_ALIAS={
  chorus:'drop', hook:'drop', refrain:'drop', inst:'drop', instrumental:'drop',
  solo:'drop', lead:'drop', high:'drop',
  mid:'verse', theme:'verse',
  start:'intro',
  end:'outro', ending:'outro',
  pre:'build', 'pre-chorus':'build', prechorus:'build', rise:'build',
  breakdown:'break', bridge:'break', low:'break',
  ambient:'quiet', silence:'quiet',
};
const chMap=n=>CH_ALIAS[String(n||'').toLowerCase().trim()]||n;
EN = EN || [];
MO = MO || [];
SP = SP || [];
...
function primaryLook(t){ if(stopAt(t))return'stop';
  ...
  const cn=chMap(chAt(t));
  if((z&&z.kind==='quiet')||['break','quiet'].includes(cn))return'quiet';
  if(cn==='drop')return'drop'; if(cn==='verse')return'verse'; return'idle'}
function buildSegs(){
...
function rolesFor(name){
  const r = ROLES[chMap(name)] || ROLES.verse;
  if(HOT_OK) return r;
...
function lookFrame(t,L){
  ...
  PARS.forEach(function(id,i){
    ...
    if(off){c=[0,0,0]}
      ...
      // a build is a RISER: layer the pars in, then add a floor that climbs
      // (accelerating, prog^2) across the whole span so the room visibly lifts
      // into the drop instead of just flickering brighter near the end.
      if(L==='build') lv = lv*lerp(0.30,1,layer(1)) + (0.12+0.30*e)*prog*prog*EX.arc;
  ...
  STROBES.forEach(function(id,i){
    ...
    if(L==='flash'){lv=1}
      ...
      // accelerating strobe across the whole build: rate climbs 1 -> 4 Hz with prog,
      // level fades in from ~40% through -- the classic tension riser into the drop
      lv=ss(0.40,0.96,prog)*((G.left===(((bx%2)+2)%2===0))?1:0.55)*Math.min(1,grow);
      hz=Math.min(CAP,(1.0+3.2*prog)*STROBE_HZ);}
  ...
  WASHES.forEach(function(id,i){
    ...
    if(off){c=[0,0,0]}
      ...
      if(L==='build') lv = lv*lerp(0.30,1,layer(1)) + (0.10+0.24*e)*prog*prog*EX.arc;
  ...
  KIND('confetti').forEach(function(id){
    let lv=0;
    // the climactic drops only: the final drop, plus the single biggest by size --
    // confetti everywhere is confetti nobody notices, but the last chorus earns it
    const dm=MO.filter(m=>m.kind==='drop');
    if(dm.length){
      const cues=[dm[dm.length-1].at];
      const big=dm.slice().sort((a,b)=>(b.v||0.9)-(a.v||0.9))[0];
      if(big && cues.indexOf(big.at)<0) cues.push(big.at);
      for(const cue of cues) if(t>=cue && t-cue<BAR*4) lv=Math.max(lv,1-ss(0,BAR*4,t-cue));
    }
    F.push({id:id,level:+cl(lv,0,1).toFixed(3),
            note:'the final drop and the biggest one'})});
