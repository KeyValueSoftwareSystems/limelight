...
class Session:
    ...
    def position_at(self, t):
        ...
        return {"bar": self.first_bar + int(math.floor(i / self.bpb)),
                "beat": _fx(_mod(i, self.bpb) + 1, 4),
                "before_first_beat": t < self.first}

    def seconds_at(self, bar, beat=1):
        return (self.first + (bar - self.first_bar) * self.bar_s
    ...
    # ---- sections, in layers ----------------------------------------------
    def _at(self, q):
        return (q["bar"] - self.first_bar) * self.bpb + (_or(q.get("beat"), 1) - 1)
