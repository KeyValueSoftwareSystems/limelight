...
def brief_for(song, overview, effects_block, hub, rig="arc4-head"):
    ...
    return "\n".join(
        [
            ...
            "",
            "HOW THIS SHOW WILL BE SCORED",
            "",
            "A deterministic referee runs over the baked DMX frames and asks nine",
            "questions. A reference show built by hand answers all nine. Aim at these",
            "directly - they are measurements of the frames, not opinions:",
            "",
            "  on the grid       brightness rises land within 60ms of a measured beat",
            "                    at least 2 sd above a circular-shift null",
            "  goes dark         between 4% and 30% of frames sit under 12 DMX",
            "  held back         at most 55% of frames sit in the middling band",
            "  not a wall        at least 45% of frames have lamps differing from",
            "                    each other by more than 20 DMX",
            "  never parks       no unchanging stretch longer than 6 seconds",
            "  peak on peak      the show's loudest 5s is within 6s of the recording's",
            "  loudest where     that stretch is at least 75% of the show's own maximum",
            "  moments land      at least 60% of moments at intensity 0.70+ get a",
            "                    visible change, brighter OR darker - at an exit, light",
            "                    going away is the correct answer",
            "  every lamp works   the busiest lamp is lit at most 4 points more of the",
            "                     show than the quietest. A hand-built show sits at 0.9;",
            "                     binding `follow` to extent `inner` in most sections",
            "                     put ours at 7.3 and an audience reads the same two",
            "                     lamps being on all night as a rig that is stuck.",
            "                     Move the extent around between sections.",
            "  colour carries     no single colour covers more than 72% of lit time. A",
            "  the form           show that is one colour end to end fails, however",
            "                    disciplined that colour is. Give the sections",
            "                    different colours so the form can be read off the rig.",
            "",
            "CRAFT RULES THAT CAME OUT OF MEASURING",
            "",
            "  COMPRESS THE ARC, DO NOT COPY IT. Tracking loudness faithfully gives a",
            "  rig that lives in the middle with nothing left to spend. Push the quiet",
            "  quieter and the loud louder. A state is always visible while the song",
            "  sounds - floor it near 0.34 - and a state never reaches full: cap states",
            "  near 0.92 so only a gesture ever reaches 255. Without that cap an",
            "  ordinary bar looks exactly like the loudest bar in the song.",
            "",
            "  INK TRACKS ENERGY. Effects differ in how much of the rig they light. A",
            "  chase or a travelling pulse lights a small fraction of the lamps; a wash",
            "  or a breathe lights all of them. Putting a chase on the climax makes the",
            "  room go dark at the loudest bar. One travelling lamp under a solo voice,",
            "  half the rig when the group answers, the whole rig at the climax.",
            "",
            "  NOTHING IS BOTH BRIGHT AND VARIED. Every full-rig look puts all lamps in",
            "  one state; every varied look stays dim on average. Above three quarters",
            "  energy, layer two bindings - a full-ink bed carrying the room and a",
            "  half-ink pattern over it. That is a wash plus a chase, the oldest",
            "  two-layer rig there is. Bindings composite by taking the brighter per",
            "  lamp, so a pattern laid over a bright bed cannot stand out; if you want",
            "  the pattern read, keep the bed's amount down in that section.",
            "",
            "  REPEATS RHYME. Pick the look from the section's identity, not from a",
            "  rotating list. Three choruses should be recognisably the same thing;",
            "  energy picks which member of the family. Spend vocabulary ACROSS",
            "  sections, not within one. A show whose repeats do not rhyme reads as a",
            "  sequence of unrelated ideas however well each one is placed.",
            "",
            "  ON A 4-LAMP RIG, do not use chases in quiet sections - a chase is one",
            "  lamp there, with no texture to read. Use breathing looks instead: dark",
            "  and alive rather than dark and dead.",
            "",
            "  A REVEAL IS WHITE, and only as bright as the bar it lands on. White",
            "  always, because that is what a reveal is - but a quiet section coming",
            "  back at full reads exactly like the climax, and then the song has four",
            "  peaks and no peak.",
            "",
            "  PLACE EVERY CUE ON SOMETHING MEASURED, NEVER ON A LABEL. Where a",
            "  section is called `chorus` and the numbers under it say otherwise,",
            "  follow the numbers and say so in the `why`.",
            "",
            "WHAT TO WRITE",
            "",
