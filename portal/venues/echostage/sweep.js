...
module.exports = function sweep(params, ctx) {
  ...
  for (let i = 0; i < N; i++) {
    ...
    H.setHead(f, H.HEADS[1], { level, colour, pan: from + to - pan, tilt, gobo: params.gobo });
    H.setHead(f, H.HEADS[2], { level: level * 0.7, colour, pan: pan * 0.8 + 0.1, tilt: tilt + 0.05, gobo: params.gobo });
    H.setHead(f, H.HEADS[3], { level: level * 0.7, colour, pan: (from + to - pan) * 0.8 + 0.1, tilt: tilt + 0.05, gobo: params.gobo });
    frames.push(f);
  }
