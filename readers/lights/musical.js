...
const or_ = (v, d) => (v === undefined || v === null) ? d : v;   /* default on absence, never falsiness */
...
function firstBarOf(score) {
  ...
  const secs = (score && (score.sections || ((score.layers || {}).form || {}).spans)) || [];
...
function subsectionsOf(score) {
  ...
  if (Array.isArray(score.phrases))
    return score.phrases.map(p => norm({ bar: p.from_bar, beat: 1 }, { bar: p.to_bar + 1, beat: 1 }, p));
  return [];
}
...
function lanesOf(score) {
  ...
  const bars = score.bars || {}, curves = score.curves || {};
  const out = { from_bar: firstBarOf(score) };
  let any = false;
  for (const k of LANES) {
    const v = Array.isArray(bars[k]) ? bars[k]
      : Array.isArray(score[k]) ? score[k]
      : (curves[k] && Array.isArray(curves[k].values)) ? curves[k].values : null;
...
function stemLanesOf(score) {
  ...
  for (const s of STEMS) {
    const v = Array.isArray(bars[s]) ? bars[s] : (fromStems && Array.isArray(fromStems[s])) ? fromStems[s] : null;
    if (v) { lanes[s] = v; any = true; }
  }
  if (!any) return null;
  const from_bar = (fromStems && isNum(score.stems.from_bar)) ? score.stems.from_bar : firstBarOf(score);
...
function harmonyOf(score) {
  ...
  if (Array.isArray(bars.chord)) {
    from_bar = firstBarOf(score); chords = bars.chord; confidence = bars.chord_sure || [];
  } else if (score.harmony && Array.isArray(score.harmony.chords)) {
    from_bar = or_(score.harmony.from_bar, firstBarOf(score));
    chords = score.harmony.chords; confidence = score.harmony.confidence || [];
  } else return null;
  const k = score.key || {};
  const key = { root: or_(k.root, null), scale: or_(k.scale, null) };
  return { from_bar, chords: chords.map(c => (c ? String(c) : null)),
