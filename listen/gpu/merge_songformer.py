    ...
    for s in mine:
        ...
        if near:
            ...
            row["also_heard"] = {
                ...
                "label": best["label"],
                "same_label": best["label"] == s.get("label"),
            }
        else:
            row["also_heard"] = None
        out.append(row)
...
def main(scores_dir, sf_dir):
    ...
    for p in sorted(glob.glob(os.path.join(scores_dir, "*.score"))):
        ...
        d["sections"] = agree(d["sections"], theirs)
        ...
        same = sum(1 for s in d["sections"]
                   if s.get("also_heard") and s["also_heard"]["same_label"])
        d["sections_second_opinion"]["agreed_boundaries"] = f"{hit} of {len(d['sections'])}"
        d["sections_second_opinion"]["agreed_labels"] = f"{same} of {hit}" if hit else "0 of 0"
        json.dump(d, open(p, "w"), indent=2, ensure_ascii=False)
        touched += 1
        print("  %-26s boundaries %2d/%-2d  labels %2d/%-2d" % (
            slug, hit, len(d["sections"]), same, hit))
    print("merged into %d scores" % touched)


if __name__ == "__main__":
    main(sys.argv[1] if len(sys.argv) > 1 else "scores",
