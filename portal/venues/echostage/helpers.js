"use strict";
/* Shared helpers for the echostage DMX effect functions.
   Rig: 20 × par7 (7ch each, addresses 1-140) + 4 × head13 (13ch each, addresses 141-192).
   Total: 192 DMX channels, universe 0.

   Layout zones:
     back_arc   par_01..par_08   curved upstage wash bar
     side_L     par_09..par_10   left tower (low + high)
     side_R     par_11..par_12   right tower (low + high)
     front      par_13..par_16   front wash truss
     floor      par_17..par_20   floor uplights
     head_outer head_1, head_2   mid-truss L/R
     head_inner head_3, head_4   above DJ booth */
...
/* Fixture layout: 20 pars, 4 heads. Addresses are 1-based in the layout but
   DMX frame arrays are 0-based, so par_01 at address 1 starts at index 0. */
const PARS = [];
for (let i = 0; i < 20; i++) PARS.push({ id: "par_" + String(i + 1).padStart(2, "0"), offset: i * 7 });
const HEADS = [
  { id: "head_1", offset: 140 },
  { id: "head_2", offset: 153 },
  { id: "head_3", offset: 166 },
  { id: "head_4", offset: 179 },
];
...
const INNER = PARS.slice(5, 15);
const OUTER = PARS.slice(0, 5).concat(PARS.slice(15, 20));
const LEFT  = PARS.slice(0, 10);
const RIGHT = PARS.slice(10, 20);
const ENDS  = [PARS[0], PARS[19]];

const CENTRE = PARS.length % 2
  ? [PARS[(PARS.length - 1) / 2]]
  : PARS.slice(PARS.length / 2 - 1, PARS.length / 2 + 1);

/* Zone helpers for the multi-truss layout. */
const BACK_ARC = PARS.slice(0, 8);
const SIDE_L   = PARS.slice(8, 10);
const SIDE_R   = PARS.slice(10, 12);
const SIDES    = SIDE_L.concat(SIDE_R);
const FRONT    = PARS.slice(12, 16);
const FLOOR    = PARS.slice(16, 20);

const HEADS_OUTER = [HEADS[0], HEADS[1]];
const HEADS_INNER = [HEADS[2], HEADS[3]];
...
module.exports = {
  ...
  BACK_ARC, SIDE_L, SIDE_R, SIDES, FRONT, FLOOR,
  HEADS_OUTER, HEADS_INNER,
