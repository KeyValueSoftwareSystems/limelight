...
function rigging(items, W, H) {
  ...
  for (const f of items) {
    const key = Math.round(f.y * 80);
    if (!rows.has(key)) rows.set(key, []);
    rows.get(key).push(f);
  }
  rows.forEach(row => {
    const r = row.reduce((a, f) => a + f.r, 0) / row.length;
    const by = Math.min(...row.map(f => H * f.y - f.top)) - r * 0.7;
    const xs = row.map(f => W * f.x);
    const pad = r * 2.6;
    trussBar(Math.min(...xs) - pad, Math.max(...xs) + pad, by, Math.max(2, r * 0.5));
    ctx.strokeStyle = "rgba(40,47,64,0.95)";
    ctx.lineWidth = Math.max(1, r * 0.3);
    ctx.beginPath();
    for (const f of row) { ctx.moveTo(W * f.x, by); ctx.lineTo(W * f.x, H * f.y - f.top * 0.86); }
    ctx.stroke();
  });
}
...
function headBody(x, y, r, rot) {
  ctx.fillStyle = "rgba(58,67,88,0.95)";
  ctx.fillRect(x - r * 0.5, y - r * 3.1, r, r * 0.7);

  ctx.strokeStyle = "rgba(60,69,92,0.95)";
  ctx.lineWidth = Math.max(1.4, r * 0.3);
  ctx.beginPath();
  ctx.moveTo(x - r * 1.3, y + r * 0.45);
  ctx.lineTo(x - r * 1.3, y - r * 2.3);
  ctx.lineTo(x + r * 1.3, y - r * 2.3);
  ctx.lineTo(x + r * 1.3, y + r * 0.45);
  ctx.stroke();
  ...
  can.addColorStop(0, "#070910");
  can.addColorStop(0.32, "#333b51");
  can.addColorStop(0.6, "#1a1f2d");
  can.addColorStop(1, "#080a11");
  ctx.fillStyle = can;
  ctx.beginPath();
  ctx.moveTo(-r * 0.98, -r * 0.05);
  ctx.lineTo(r * 0.98, -r * 0.05);
  ctx.lineTo(r * 0.82, r * 1.85);
  ctx.lineTo(-r * 0.82, r * 1.85);
  ctx.closePath(); ctx.fill();

  ctx.strokeStyle = "rgba(140,154,190,0.3)";
  ctx.lineWidth = Math.max(0.8, r * 0.09);
  ctx.beginPath();
  ctx.moveTo(-r * 0.64, r * 1.8); ctx.lineTo(-r * 0.76, -r * 0.02);
  ctx.moveTo(-r * 0.98, -r * 0.05); ctx.lineTo(r * 0.98, -r * 0.05);
  ctx.stroke();

  const bore = ctx.createRadialGradient(0, 0, 0, 0, 0, r * 0.86);
  bore.addColorStop(0, "#0b0e16");
  bore.addColorStop(1, "#030408");
  ctx.fillStyle = bore;
  ctx.beginPath(); ctx.ellipse(0, -r * 0.02, r * 0.86, r * 0.34, 0, 0, TAU); ctx.fill();
  ctx.restore();

  ctx.fillStyle = "rgba(102,113,145,0.9)";
  ctx.beginPath(); ctx.arc(x - r * 1.3, y, Math.max(1, r * 0.21), 0, TAU); ctx.fill();
  ctx.beginPath(); ctx.arc(x + r * 1.3, y, Math.max(1, r * 0.21), 0, TAU); ctx.fill();
}

function bezel(x, y, r, k) {
  ctx.strokeStyle = "rgba(7,9,15," + (0.72 - 0.34 * Math.min(1, k || 0)).toFixed(3) + ")";
  ctx.lineWidth = Math.max(1, r * 0.17);
  ctx.beginPath(); ctx.arc(x, y, r * 1.02, 0, TAU); ctx.stroke();
  ctx.strokeStyle = "rgba(160,174,210,0.26)";
  ctx.lineWidth = Math.max(0.6, r * 0.07);
  ctx.beginPath(); ctx.arc(x, y, r * 1.1, Math.PI * 1.12, Math.PI * 1.82); ctx.stroke();
}
...
function bandedCone(wBottom, wTop, L, n, phase, duty) {
  ...
  for (let i = 0; i < n; i++) {
    const t0 = (i + phase) / n;
    if (t0 >= 1) continue;
    const t1 = Math.min(1, t0 + duty / n);
    const w0 = wBottom + (wTop - wBottom) * t0, w1 = wBottom + (wTop - wBottom) * t1;
    ctx.moveTo(-w0, -t0 * L);
    ctx.lineTo(w0, -t0 * L);
    ctx.lineTo(w1, -t1 * L);
    ctx.lineTo(-w1, -t1 * L);
