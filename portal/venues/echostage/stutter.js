...
module.exports = function stutter(params, ctx) {
  ...
  for (let i = 0; i < N; i++) {
    ...
    H.setHead(f, H.HEADS[0], { level: on ? level * decay : 0, colour, pan: 0.55, tilt: 0.44 });
    H.setHead(f, H.HEADS[1], { level: on ? level * decay : 0, colour, pan: 0.77, tilt: 0.44 });
    H.setHead(f, H.HEADS[2], { level: on ? level * decay * 0.7 : 0, colour, pan: 0.62, tilt: 0.48 });
    H.setHead(f, H.HEADS[3], { level: on ? level * decay * 0.7 : 0, colour, pan: 0.70, tilt: 0.48 });
    frames.push(f);
  }
