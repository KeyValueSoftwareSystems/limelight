"use strict";
/* wire.js -- intents -> 41-channel DMX frames. The one place the wire rules live,
   shared by bake.js (a whole show) and preview.js (one sequence):
     ...
     - the head's pose is slew-limited as ONE 16-bit value -- coarse and fine
...
function toLightsFrames(ticks, layout, opts) {
  opts = opts || {};
  const W = opts.width || 41, gammaExp = opts.gamma || GAMMA, maxStep = opts.maxStep || 7;
  const addrOf = {};
  ...
  const driven = [];   // per frame: did the head's intent carry a pan / a tilt?
  const frames = ticks.map(tk => {
    const f = new Array(W).fill(0);
    let d = { pan: false, tilt: false };
    for (const fx of tk.fixtures) {
      ...
      if (head && fx.id === head.id) d = { pan: !!(fx.intent && fx.intent.pan != null), tilt: !!(fx.intent && fx.intent.tilt != null) };
    }
...
module.exports = { toLightsFrames, GAMMA, PARK };
