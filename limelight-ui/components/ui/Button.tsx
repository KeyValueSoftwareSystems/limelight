...
const variants: Record<Variant, string> = {
  ...
  ghost: `${base} inline-flex items-center h-[var(--hit)] px-[14px] bg-white/[0.03] border border-solid border-white/[0.06] rounded-[var(--radius-sm)] text-[12px] font-medium text-ink-dim hover:text-ink hover:bg-white/[0.06] hover:border-white/[0.12] active:scale-[0.98]`,
  primary: `${base} inline-flex items-center h-[var(--hit)] px-[18px] border-0 rounded-[var(--radius-sm)] text-[13px] font-semibold text-[#0C0D12] hover:brightness-110 active:scale-[0.97] active:brightness-95`,
...
export const Button = forwardRef<HTMLButtonElement, ButtonProps>(
  ({ variant = "default", active, wide, className = "", style, ...props }, ref) => {
    ...
    const primaryStyle = variant === "primary" && !active
      ? { background: "linear-gradient(180deg, #FBBF24 0%, #F59E0B 100%)", boxShadow: "0 1px 2px rgba(0,0,0,0.2), inset 0 1px 0 rgba(255,255,255,0.2)" }
      : undefined;
