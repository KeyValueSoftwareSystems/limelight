...
def onset_strength(path, sr=22050):
...
def onset_peaks(a, rate=RATE, refractory=0.30):
    ...
    cands = [(a[i], i / rate) for i in range(1, len(a) - 1)
...
def score(path):
    ...
    return {
        ...
        "cut_align": cut_alignment(cuts, peaks, dur),
...
def main():
    ...
    print(f"{'file':40} {'cuts/m':>6} {'medshot':>7} {'gini':>6} | "
          f"{'on-onset':>9} {'null':>7} {'z':>6} {'pct':>6} | {'curve z':>7}")
    for r in rows:
        if r.get("error"):
            print(f"{os.path.basename(r['file'])[:40]:40} {r['error']}")
            continue
        ca = r.get("cut_align") or {}
        cv = r.get("curve_align") or {}
        nan = float("nan")
        print(f"{os.path.basename(r['file'])[:40]:40} {r['cuts_per_min']:6.1f} "
              f"{r.get('shot_s_median', 0):7.2f} "
              f"{(r.get('gini') if r.get('gini') is not None else nan):6.3f} | "
              f"{ca.get('hit_rate', nan):9.3f} {ca.get('null_hit_rate', nan):7.3f} "
              f"{ca.get('z', nan):6.2f} {ca.get('percentile', nan):5.1f}% | "
              f"{cv.get('z', nan):7.2f}")
    print()
    print(f"on-onset is the share of detected cuts landing within {TOL*1000:.0f} ms of an")
    print("onset peak, both found in the rendered file. null is the same share")
    print("for rotated copies of those cuts -- identical count and spacing, no")
    print("relationship to the audio. pct is the share of rotations beaten.")
    print()
